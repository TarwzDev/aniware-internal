...
namespace movement
{
	...
	void strafe( )
	{
		...
		float velocity_yaw = ctx::client.local->get_velocity( ).y, 
			rotation = std::clamp< float >( math::rad2deg( std::atan2( 15.f, ctx::client.local->get_velocity( ).length_2d( ) ) ), 0.f, 90.f );
		...
		float cos_rot = std::cos( math::deg2rad( viewangles.y - ideal_yaw_rotation ) ), sin_rot = std::sin( math::deg2rad( viewangles.y - ideal_yaw_rotation ) );

		float forwardmove = ctx::client.cmd->forwardmove, sidemove = ctx::client.cmd->sidemove;

		if ( config::get< int >( ctx::cfg.movement_strafe_type ) )
		{
